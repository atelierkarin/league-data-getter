...
match_id = '2020A0011614'
url = "http://www.jfl.or.jp/jfl-pc/view/s.php?a=1576&f={}_spc.html".format(match_id)
...
for df in dfs:
  if counter >= 1 and data_list_counter < len(data_list):
    if (data_list[data_list_counter] == "HG" or data_list[data_list_counter] == "AG") and len(df.columns) == 3:
      if data_list[data_list_counter] == "HG" and df.shape[0] == home_goal:
        home_team_goal_info = df.copy()
        data_list_counter += 1
      elif data_list[data_list_counter] == "AG" and df.shape[0] == away_goal:
